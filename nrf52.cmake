...
#	endif()
	...
	if (HAS_BOOTLOADER)
		...
		set(MERGE_HEX_CMD_BYPRODUCTS
				${SETTINGS_HEX_CMD_BYPRODUCTS}
				${BOOT_HEX_OUT}
				)
	else()
		set(MERGE_HEX_CMD)
...
#			BYPRODUCTS  ${OUT_HEX} ${MERGE_HEX_CMD_BYPRODUCTS}
			)
...
#		set(OPENOCD_CFG board/nordic_nrf52_dk.cfg)
	...
	message(STATUS "SETTINGS_HEX_CMD_BYPRODUCTS = ${SETTINGS_HEX_CMD_BYPRODUCTS}")
	add_custom_target(Flash
			DEPENDS ${TARGET}

			COMMAND echo ${DELFILE_CMD} ${APP_HEX_FILE}
			COMMAND ${DELFILE_CMD} ${APP_HEX_FILE}
			COMMAND ${CMAKE_OBJCOPY} -Oihex ${CMAKE_BINARY_DIR}/${TARGET} ${APP_HEX_FILE}
			COMMAND echo ${APP_HEX_FILE} created
			${SETTINGS_HEX_CMD}
			COMMAND ${OPENOCD_BIN} ${OPENOCD_EXTRA_CMD} -f ${OPENOCD_CFG} ${OPENOCD_SCRIPT_CMD}  ${OPENOCD_FLASH_CMD} -c reset -c exit
			WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR} USES_TERMINAL
#			BYPRODUCTS tmp-${APP_HEX_FILE} ${SETTINGS_HEX_CMD_BYPRODUCTS}
			VERBATIM)
	...
	add_custom_target(Flash-debug
			...
			COMMAND echo ${APP_HEX_FILE} created
			${SETTINGS_HEX_CMD}
			COMMAND ${OPENOCD_BIN} ${OPENOCD_EXTRA_CMD} -f ${OPENOCD_CFG} ${OPENOCD_SCRIPT_CMD}  ${OPENOCD_FLASH_CMD} -c reset -c halt -c exit
			WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
#			BYPRODUCTS ${APP_HEX_FILE} ${SETTINGS_HEX_CMD_BYPRODUCTS}
			USES_TERMINAL)
...
FUNCTION(ADD_HEX_BIN_TARGETS TARGET OUTPUT)

	get_filename_component(OUTPUT_PATH ${OUTPUT} DIRECTORY)
	message(STATUS "Output directory ${OUTPUT_PATH}")
	set(TARGET_HEX_FILE ${OUTPUT}.hex)
	set(OUT_HEX_FILE ${OUTPUT}-FULL.hex)
	set(TARGET_HEX_FILE ${OUTPUT}.hex PARENT_SCOPE)
	set(TARGET_BIN_FILE ${OUTPUT}.bin PARENT_SCOPE)

	add_custom_command(TARGET ${TARGET} POST_BUILD
			COMMAND ${CMAKE_COMMAND} -E make_directory ${OUTPUT_PATH}
			COMMAND ${CMAKE_OBJCOPY} ${TARGET} ${OUTPUT}.elf
			COMMAND ${CMAKE_OBJCOPY} -Oihex ${TARGET} ${OUTPUT}.hex
			COMMAND ${CMAKE_OBJCOPY} -Obinary ${TARGET} ${OUTPUT}.bin
			BYPRODUCTS ${OUTPUT}.hex ${OUTPUT}.bin ${OUTPUT}.elf
			)
	message(STATUS "ADD_HEX_BIN_TARGETS : ${TARGET} ${TARGET_HEX_FILE}" )
	message(STATUS "Flash-Segger Ouputs : ${OUT_HEX_FILE}" )
	NRF_MERGE_HEX(${TARGET} ${TARGET_HEX_FILE} ${OUT_HEX_FILE})
	add_custom_target(Flash-Segger
			DEPENDS ${TARGET}
			COMMAND nrfjprog -f NRF52 --program ${OUT_HEX_FILE} --chiperase --reset --log
			WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR} USES_TERMINAL
			BYPRODUCTS ${OUT_HEX_FILE}
			VERBATIM)
ENDFUNCTION()
